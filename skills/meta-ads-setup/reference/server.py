...
async def _get_insights(
    ...
    try:
        ...
        if not data:
            return f"No insights data found for the selected period."
...
async def campaign_insights(
    ...
    level = "campaign"
    return await _get_insights(ctx, path, level, date_preset, date_start, date_end, breakdowns, limit, after)
...
@mcp.tool(
    ...
    description="Create a new campaign. Requires name, objective, and status. Optionally set daily or lifetime budget.",
...
async def create_campaign(
    ...
    Args:
        ...
        special_ad_categories: JSON array of special categories like ["HOUSING"], ["EMPLOYMENT"], ["CREDIT"]. Default [].
    """
    try:
        data = {
            "name": name,
...
@mcp.tool(
    ...
    description="Create a new ad set. Requires campaign ID, name, targeting, budget, and optimization goal.",
    annotations={"readOnlyHint": False, "destructiveHint": False, "idempotentHint": False, "openWorldHint": True},
)
async def create_adset(
    ...
    Args:
        ...
        name: Ad set name.
        daily_budget: Daily budget in cents (e.g. "5000" = $50.00).
        optimization_goal: IMPRESSIONS, LINK_CLICKS, REACH, LANDING_PAGE_VIEWS, OFFSITE_CONVERSIONS, LEAD_GENERATION, APP_INSTALLS.
        ...
        targeting_interests: JSON array of interest objects e.g. [{"id":"6003139266461","name":"Movies"}].
        start_time: ISO 8601 datetime (e.g. 2025-01-15T00:00:00-0800).
        end_time: ISO 8601 datetime.
        status: ACTIVE or PAUSED (default PAUSED).
        bid_amount: Bid amount in cents (required for LOWEST_COST_WITH_BID_CAP).
    """
    try:
...
@mcp.tool(
    name="meta_create_ad",
    description="Create a new ad by linking an existing creative to an ad set.",
    annotations={"readOnlyHint": False, "destructiveHint": False, "idempotentHint": False, "openWorldHint": True},
)
async def create_ad(
    ctx: Context,
    ...
    Args:
        ...
        name: Ad name.
        creative_id: The creative ID to use. Get this from existing creatives or create one first.
        status: ACTIVE or PAUSED (default PAUSED).
    """
    try:
        data = {
...
@mcp.tool(
    ...
    description="Create a custom audience. Supports WEBSITE, CUSTOM (customer list), and ENGAGEMENT subtypes.",
    annotations={"readOnlyHint": False, "destructiveHint": False, "idempotentHint": False, "openWorldHint": True},
)
async def create_audience(
    ...
    Args:
        ...
        rule: JSON rule for WEBSITE audiences. E.g. {"inclusions":{"operator":"or","rules":[{"event_sources":[{"id":"PIXEL_ID","type":"pixel"}],"retention_seconds":2592000}]}}.
